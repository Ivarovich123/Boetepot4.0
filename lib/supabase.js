  ...
  try {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select(`
        ...
        amount,
        date,
        player_name:players!inner(name),
        reason_description:reasons!inner(description)
      `)
      .order('date', { ascending: false })
      .limit(10);
    
    if (error) {
      console.error('Error fetching public fines:', error);
      handleError(error, 'getPublicFines');
    }
    
    console.log('Raw fines data:', data);
    ...
    const formattedFines = data.map(fine => ({
      ...
      player_name: fine.player_name?.[0]?.name || 'Onbekend',
      reason_description: fine.reason_description?.[0]?.description || 'Onbekend'
    }));
