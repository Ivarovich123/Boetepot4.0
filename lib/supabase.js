...
async function getPlayerTotals() {
  try {
    ...
    if (finesError) {
      console.error('[DB] Error fetching fines:', finesError);
      return [];
    }

    console.log('[DB] Player totals raw data:', { players, fines });
    
    const totals = players.map(player => {
      const playerFines = fines.filter(fine => fine.player_id === player.id);
      const total = playerFines.reduce((sum, fine) => sum + (fine.amount || 0), 0);
      return {
        name: player.name,
        total,
    ...
    console.log('[DB] Player totals formatted:', totals);
    return totals;
  } catch (error) {
    console.error('[DB] Error in getPlayerTotals:', error);
    return [];
  }
}
...
async function getTotalFines() {
  try {
    console.log('[DB] Fetching total fines...');
    const { data, error } = await supabaseAdmin
      .from('fines')
      .select('amount');
    
    if (error) {
      console.error('[DB] Error in getTotalFines:', error);
      return 0;
    }

    if (!data || !Array.isArray(data)) {
      console.error('[DB] Invalid data returned from getTotalFines:', data);
      return 0;
    }

    console.log('[DB] Total fines raw data:', data);
    const total = data.reduce((sum, fine) => sum + (fine.amount || 0), 0);
    console.log('[DB] Total fines calculated:', total);
    return total;
  } catch (error) {
    console.error('[DB] Error in getTotalFines:', error);
    return 0;
...
async function getPublicFines() {
  try {
    console.log('[DB] Fetching public fines...');
    const { data, error } = await supabaseAdmin
      .from('fines')
      .select(`
        id,
    ...
    if (error) {
      console.error('[DB] Error in getPublicFines:', error);
      handleError(error, 'getPublicFines');
      return [];
    }

    if (!data || !Array.isArray(data)) {
      console.error('[DB] Invalid data returned from getPublicFines:', data);
      return [];
    }

    console.log('[DB] Public fines raw data:', data);
    ...
    console.log('[DB] Public fines formatted:', formattedFines);
    return formattedFines;
  } catch (error) {
    console.error('[DB] Error in getPublicFines:', error);
    return [];
  }
