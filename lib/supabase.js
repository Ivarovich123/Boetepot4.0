...
async function getPlayerTotals() {
  try {
    ...
    return data.map(player => ({
      id: player.id,
      name: player.name,
      total: player.fines?.reduce((sum, fine) => sum + (fine.amount || 0), 0) || 0
    }))
...
async function getAllFines() {
  try {
    console.log('[DB] Fetching all fines...');
    const { data, error } = await supabase
      .from('fines')
      .select(`
        id,
        ...
        reasons:reason_id (id, description)
      `)
      .order('date', { ascending: false });
      
    if (error) {
      console.error('[DB] Error fetching all fines:', error);
      throw error;
    }
    
    console.log('[DB] All fines data:', data);
    return data.map(fine => ({
      id: fine.id,
      amount: fine.amount,
      date: fine.date,
      player_id: fine.players?.id,
      player_name: fine.players?.name,
      reason_id: fine.reasons?.id,
      reason_description: fine.reasons?.description
    }));
  } catch (error) {
    handleError(error, 'getAllFines');
  }
}

async function getFineById(id) {
  try {
    console.log('[DB] Fetching fine by ID:', id);
    const { data, error } = await supabase
      .from('fines')
      .select(`
        id,
    ...
    if (error) {
      console.error('[DB] Error fetching fine by ID:', error);
      throw error;
    }
    ...
    handleError(error, 'getFineById');
  }
}

async function updateFine(id, fine) {
  try {
    console.log('[DB] Updating fine:', id, fine);
    ...
    if (error) {
      console.error('[DB] Error updating fine:', error);
      throw error;
    }
    ...
    console.log('[DB] Updated fine:', data);
    return {
      id: data.id,
      amount: data.amount,
      date: data.date,
    ...
    handleError(error, 'updateFine');
  }
}

async function deleteFine(id) {
  try {
    console.log('[DB] Deleting fine:', id);
    ...
    if (error) {
      console.error('[DB] Error deleting fine:', error);
      throw error;
    }
    
    console.log('[DB] Successfully deleted fine:', id);
  } catch (error) {
